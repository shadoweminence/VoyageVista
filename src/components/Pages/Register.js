...
export default function Register(props) {
    ...
    if (password !== cpassword) {
      ...
      try {
          const response = await fetch("http://localhost:5000/api/auth/createuser", {
            ...
            body: JSON.stringify({name,email,password,confirmPassword : cpassword})
          });
          ...
          props.showAlert("Registration  failed: " + (error.response?.data?.error || error.message), "danger");
      }
  };
  ...
  <div className="mb-3">
    ...
    <input type="password" name="cpassword" className="form-control" id="cpassword" minLength={5} onChange={onChange}/>
  </div>
  ...
  <button type="submit" className="btn btn-primary" >Submit</button>
     ...
     <Link className="abc" to="/">Log In</Link> </p>
  </div>
  </>
  )
}
